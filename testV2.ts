...
import { abi as module_abi } from './artifacts/contracts/WhitelistingModuleV2.sol/WhitelistingModuleV2.json'
...
async function testt(){
    ...
    // const V3swapRouterIface = new ethers.utils.Interface(V3swapRouterABI)
    // console.log(V3swapRouterIface.getSighash('multicall'))
    let data = iface.encodeFunctionData('addNewAllowance',
    ...
    //add operator
    data = iface.encodeFunctionData('addNewOperator', [ await safeOwner1.getAddress() ])
    safeTransactionData = {
        to: module.address,
        value: '0',
        data: data
    }
    whitelistTransaction = await safeSdk.createTransaction({ safeTransactionData })
    signedSafeTransaction = await safeSdk.signTransaction(whitelistTransaction)
    signedSafeTransaction2 = await safeSdk2.signTransaction(signedSafeTransaction)
    txResponse = await safeSdk.executeTransaction(signedSafeTransaction2)
    await txResponse.transactionResponse?.wait()
